...
Ceteris Paribus is a term that means .hi[all else equal.]
...
At first glance, this may seem counter-intuitive to you, but it's actually a very necessary assumption in order to understand how the economy.
...
However, you .hi[won't know] if it was the last button/dial alone that had the effect or some combination

--

The other option is to turn the dials/push the buttons one at a time/a few at a time, and record the results. This lets you isolate effects from specific buttons
---
## The Modeled Economy
...
Another common assumption is that we will assume humans are .hi[rational creatures] and reasonably will try to maximize their benefits whenever they make choices.
...
This is a simplifying assumption, but has proven to generate good predictions of behavior. At the end of the course, we'll explore how humans sometimes do not behave this way in real life.

--

In the history of economics however, this assumption has been fairly robust.
...
First though, I'm going to review how .hi[graphs] work. You guys likely know this, but I want to be thorough.
---

## PPF modifications

```{r, plot, out.width = '30%', out.height='30%', echo = F}
ppfunc <- function(x) sqrt(-x^2 + 100)

p = ggplot(data = data.frame(x=0), aes(x=x))

p_1 = p + stat_function(fun = ppfunc) + xlim(0,10) + ylim(0,10) + xlab("Fish") + ylab("Water") + theme_minimal() + stat_function(fun = ppfunc, xlim = c(0,10), geom = 'area', alpha = .25, fill = 'green')
